...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (ctx->CreateStateBlock(D3DSBT_ALL, &d3d9_state_block) == D3D_OK)
    ...
    if (g_pVB == NULL || g_VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (ctx->CreateVertexBuffer(g_VertexBufferSize * sizeof(CUSTOMVERTEX),
            D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_CUSTOMVERTEX,
            D3DPOOL_DEFAULT, &g_pVB, NULL) != D3D_OK)
            goto end;
    }
    if (g_pIB == NULL || g_IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if (ctx->CreateIndexBuffer(g_IndexBufferSize * sizeof(ImDrawIdx),
            D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
            sizeof(ImDrawIdx) == 2 ? D3DFMT_INDEX16 : D3DFMT_INDEX32,
            D3DPOOL_DEFAULT, &g_pIB, NULL) != D3D_OK)
            goto end;
    }

    // Lock and fill buffers
    {
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                ctx->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0,
