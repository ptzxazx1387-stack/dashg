...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data) {
    ...
    // Backup state
    IDirect3DStateBlock9* d3dStateBlock = NULL;
    ctx->CreateStateBlock(D3DSBT_ALL, &d3dStateBlock);
    if (d3dStateBlock) d3dStateBlock->Capture();

    CUSTOMVERTEX* vtx_dst;
    ImDrawIdx* idx_dst;
    if (g_pVB->Lock(0, (UINT)(draw_data->TotalVtxCount * sizeof(CUSTOMVERTEX)), (void**)&vtx_dst, D3DLOCK_DISCARD) != D3D_OK) {
        if (d3dStateBlock) d3dStateBlock->Release();
        return;
    }
    if (g_pIB->Lock(0, (UINT)(draw_data->TotalIdxCount * sizeof(ImDrawIdx)), (void**)&idx_dst, D3DLOCK_DISCARD) != D3D_OK) { 
        g_pVB->Unlock(); 
        if (d3dStateBlock) d3dStateBlock->Release();
        return; 
    }
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != NULL) {
                ...
                ctx->SetTexture(0, (IDirect3DTexture9*)pcmd->GetTexID());
                ctx->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, vtx_offset + pcmd->VtxOffset, 0, (UINT)cmd_list->VtxBuffer.Size, idx_offset + pcmd->IdxOffset, pcmd->ElemCount / 3);
            }
        }
